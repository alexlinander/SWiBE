...
class Specs(Dataset):
    def __init__(self, data_dir, subset, dummy, shuffle_spec, num_frames, normalize="noisy", spec_transform=None,
            stft_kwargs=None, **ignored_kwargs):

        self.clean_files = sorted(glob(join(data_dir, subset) + '/clean/*.wav'))
        self.noisy_files = [sub.replace("/clean/", "/noisy/") for sub in self.clean_files]
        if subset=="train":
            self.noisy16k_files = sorted(glob(join(data_dir, subset) + '/noisy_16k/*.wav'))
