...
def lsd(s, s_hat):
    ...
    spec = librosa.core.spectrum.stft(s, n_fft = 512, hop_length = 128, win_length = 256, window=window)
    spec_hat = librosa.core.spectrum.stft(s_hat, n_fft = 512, hop_length = 128, win_length = 256, window=window)
    spec = np.log(abs(spec))
    spec_hat = np.log(abs(spec_hat))
    lsd_ = np.mean([((np.mean((spec_-spec_pred_)**2))**(1/2)) for spec_,spec_pred_ in zip(spec.T,spec_hat.T)])
    #print(lsd)
